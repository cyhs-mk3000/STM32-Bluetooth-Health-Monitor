...
/*
	在访问I2C设备前，请先调用 MPU6050_IIC_CHECKDEVICE() 检测I2C设备是否正常，该函数会配置GPIO
*/
...
#define MPU6050_I2C_SCL_PIN		GPIO_Pin_9		/* 连接到SCL时钟线的GPIO */
#define MPU6050_I2C_SDA_PIN		GPIO_Pin_8			/* 连接到SDA数据线的GPIO */
...
/* 定义读写SCL和SDA的宏，已增加代码的可移植性和可阅读性 */
	...
	#define MPU6050_I2C_SCL_1()  MPU6050_GPIO_PORT_I2C->BSRR = MPU6050_I2C_SCL_PIN				/* SCL = 1 */
	#define MPU6050_I2C_SCL_0()  MPU6050_GPIO_PORT_I2C->BRR = MPU6050_I2C_SCL_PIN				/* SCL = 0 */
	
	#define MPU6050_I2C_SDA_1()  MPU6050_GPIO_PORT_I2C->BSRR = MPU6050_I2C_SDA_PIN				/* SDA = 1 */
	#define MPU6050_I2C_SDA_0()  MPU6050_GPIO_PORT_I2C->BRR = MPU6050_I2C_SDA_PIN				/* SDA = 0 */
	
	#define MPU6050_I2C_SDA_READ()  ((MPU6050_GPIO_PORT_I2C->IDR & MPU6050_I2C_SDA_PIN) != 0)	/* 读SDA口线状态 */
#endif
...
/*
*********************************************************************************************************
*	函 数 名: MPU6050_IIC_READ_BYTE
*	功能说明: CPU从I2C总线设备读取8bit数据
*	形    参：无
*	返 回 值: 读到的数据
*********************************************************************************************************
*/
uint8_t MPU6050_IIC_READ_BYTE(u8 ack)
{
	uint8_t i;
	uint8_t value;
...
uint8_t MPU6050_IIC_CHECKDEVICE(uint8_t _Address)
{
	...
	MPU6050_IIC_GPIO_CFG();		/* 配置GPIO */
	
	MPU6050_IIC_START();		/* 发送启动信号 */

	/* 发送设备地址+读写控制bit（0 = w， 1 = r) bit7 先传 */
	MPU6050_IIC_SEND_BYTE(_Address|I2C_WR);
	ucAck = MPU6050_IIC_WAIT_ACK();	/* 检测设备的ACK应答 */
